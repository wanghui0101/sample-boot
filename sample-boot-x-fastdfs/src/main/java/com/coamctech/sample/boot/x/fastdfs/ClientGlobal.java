...
package com.coamctech.sample.boot.x.fastdfs;
...
import com.coamctech.sample.boot.x.fastdfs.commons.IniFileReader;
import com.coamctech.sample.boot.x.fastdfs.commons.MyException;
 ...
 * 添加从输入流读取配置文件的方式
 * 
 * @author Happy Fish / YuQing / Wanghui
...
public class ClientGlobal {
	...
	public static void init(String conf_filename) throws FileNotFoundException,
		...
		init(new IniFileReader(conf_filename));
	}
	
	public static void init(InputStream conf) throws FileNotFoundException,
		IOException, MyException {
		init(new IniFileReader(conf));
	}
	 ...
	 *            config filename
	 */
	private static void init(IniFileReader iniReader) throws FileNotFoundException,
			IOException, MyException {
		...
		if (szTrackerServers == null) {
			throw new MyException("item \"tracker_server\" in " + iniReader.getConfFilename()
					+ " not found");
		}
